...
for target_class in target_classes:
  ...
  for intensity in attention_levels:
    ...
    # check whether the script has already run for this attention x intensity combination
    combo_name = 'files/confusion_matrices/class{}_intensity{}.pickle'.format(target_class, int(intensity*1000))
    existing_matrix_files = files = glob.glob('files/confusion_matrices/class*.pickle')
    # only run the rest of the script if the combination is not saved as a confusion matrices yet
    if combo_name not in existing_matrix_files:
