...
very_low, low, mid, high, very_high = np.quantile(per_class_recall, [0.1, 0.25, 0.5, 0.75, 0.9])
quantiles = np.where(per_class_recall<=very_low, 'q1',
                     np.where((per_class_recall>very_low) & (per_class_recall<=low), 'q2',
                              np.where((per_class_recall>low) & (per_class_recall<=mid), 'q3',
                                       np.where((per_class_recall>mid) & (per_class_recall<=high), 'q4',
                                                np.where((per_class_recall>high) & (per_class_recall<=very_high), 'q5',
                                                        np.where(per_class_recall>very_high, 'q6', ''))))))
...
recall_q5 = np.where(quantiles == 'q5')[0]
recall_q6 = np.where(quantiles == 'q6')[0]

# write quantiles to a pickle file
with open('files/all_class_recall_quantiles_6quantiles.pickle', 'wb') as file:
    pickle.dump([recall_q1, recall_q2, recall_q3, recall_q4, recall_q5, recall_q6], file)

# select 20 classes from each recall quantile
np.random.seed(1)
samples_recall_q1 = np.random.choice(recall_q1, size=20, replace=False)
samples_recall_q2 = np.random.choice(recall_q2, size=20, replace=False)
samples_recall_q3 = np.random.choice(recall_q3, size=20, replace=False)
samples_recall_q4 = np.random.choice(recall_q4, size=20, replace=False)
samples_recall_q5 = np.random.choice(recall_q5, size=20, replace=False)
samples_recall_q6 = np.random.choice(recall_q6, size=20, replace=False)

# write results to a pickle file
with open('files/target_class_recall_6_quantiles.pickle', 'wb') as file:
    pickle.dump([samples_recall_q1, samples_recall_q2, samples_recall_q3, samples_recall_q4, samples_recall_q5, samples_recall_q6], file)
...
for i in target_class_summary['target_class']:
    ...
    elif i in samples_recall_q4:
        recall_quantiles.append('Q4')
    elif i in samples_recall_q5:
        recall_quantiles.append('Q5')
    else:
        recall_quantiles.append('Q6')
