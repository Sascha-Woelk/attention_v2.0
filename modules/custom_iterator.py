...
class CustomIterator(IteratorType):
    ...
    def __init__(self, n, batch_size, shuffle, seed, classes, target_class=None, sampling_rate=None, target_class_weight=None):
        self.n = n
        ...
        self.target_class_indices = None #SW
        self.non_target_class_indices = None #SW
        self.total_classes = None
        self.target_class_subclasses = None
        self.non_target_class_subclasses = None
        self.target_class_size = None
        self.non_target_class_size = None
        self.samples_per_non_target_subclass = None

    def _set_index_array(self):
        self.index_array = np.arange(self.n)
        
        # SW sort index_array into target class vs non target class
        ...
        if self.target_class_weight is not None:
            ...
            if self.sampling_rate is not None:
                ...
                for class_number in unique_classes:
                    class_indices = self.index_array[self.classes == class_number]
                    subsampled_indices = np.random.choice(class_indices, size=int(len(class_indices) * self.sampling_rate))
                    new_index_array = np.append(new_index_array, subsampled_indices)
                self.index_array = new_index_array
            if self.shuffle:
                self.index_array = np.random.permutation(self.index_array)
